...
const groupTags = groups => groups?.length ? groups.map(g => <Tag key={g.id}>{g.name}</Tag>) : <span style={{ color: '#999' }}>未分组</span>
...
export default function Users() {
  ...
  const loadGroups = useCallback(() => api.get('/api/groups').then(r => {
    ...
    message.error('加载分组失败，已禁用用户分组编辑')
  }), [])
  ...
  const saveUser = async (values, target) => {
    if (!groupsReady) {
      message.error('分组尚未加载，不能提交用户信息')
      return
    }
    try {
      if (target) await api.put(`/api/users/${target.id}`, values)
  ...
  const openEdit = target => {
    ...
    editForm.setFieldsValue({
      ...
      group_ids: groupIds(target.groups).filter(id => activeIds.has(id)),
    })
  }
  ...
  const columns = [
    ...
    { title: '分组', dataIndex: 'groups', render: groupTags },
    ...
    { title: '操作', width: 230, render: (_, target) => { const self = target.id === currentUser?.id; return <Space size={4}>
      <Button type="link" size="small" disabled={!groupsReady} icon={<EditOutlined />} onClick={() => openEdit(target)}>编辑</Button>
  ...
  const userFields = <>
    ...
    <Form.Item name="group_ids" label="所属分组"><Select mode="multiple" allowClear placeholder="请选择分组" options={activeGroupOptions(groups)} /></Form.Item>
  </>

  return <div>
    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 16 }}><h2 style={{ margin: 0 }}>用户管理</h2><Space><Button icon={<TeamOutlined />} disabled={!groupsReady} onClick={() => setGroupsOpen(true)}>分组管理</Button><Button type="primary" icon={<PlusOutlined />} disabled={!groupsReady} onClick={() => { createForm.setFieldsValue({ role: 'operator', group_ids: defaultGroupIds(groups) }); setCreateOpen(true) }}>创建用户</Button></Space></div>
    ...
    <Modal title="创建用户" open={createOpen} onCancel={() => setCreateOpen(false)} onOk={() => createForm.submit()} okButtonProps={{ disabled: !groupsReady }}><Form form={createForm} layout="vertical" onFinish={values => saveUser(values)}><Form.Item name="username" label="用户名" rules={[{ required: true }]}><Input maxLength={50} /></Form.Item><Form.Item name="password" label="密码" rules={[{ required: true, min: 4, message: '密码至少需要 4 个字符' }]}><Input.Password maxLength={200} /></Form.Item>{userFields}</Form></Modal>
