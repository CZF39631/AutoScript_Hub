...
- 当前正式版本：`v1.2.5`，Tag 指向 `7824a94f17dc4a6d9facde105af6396652e794f3`，不得移动；发布与正式部署记录见 `docs/releases/v1.2.5-发布记录.md`。
...
- 公司服务端已运行 `1.2.5` stable，数据库迁移版本为 `0005_server_settings`；容器、内外网健康检查与 SQLite 完整性已验证。
...
- 脚本运行在客户端 Agent 的本地进程中，存在 `running` 任务不应直接成为服务端升级的硬性阻断条件。兼容升级先在旧服务运行时预拉镜像，再备份并快速切换；不停止客户端或取消任务。服务端当前有约 90 秒的心跳超时看门狗，须按实际源码/配置评估维护窗口，并在恢复后核对任务状态、心跳和补传，不能宣称断连完全无影响。涉及执行协议不兼容或客户端进程变更时另行评估。
